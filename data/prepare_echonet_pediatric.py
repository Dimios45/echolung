#!/usr/bin/env python3
"""
Prepare EchoNet-Pediatric dataset for EchoJEPA probe evaluation.

Usage:
    python data/prepare_echonet_pediatric.py \
        --echonet_dir /path/to/EchoNet-Pediatric \
        --output_dir data/csv
"""
...
def main():
    parser = argparse.ArgumentParser(description="Prepare EchoNet-Pediatric for EchoJEPA")
    parser.add_argument("--echonet_dir", type=str, required=True,
                        help="Path to EchoNet-Pediatric root")
    ...
    parser.add_argument("--video_subdir", type=str, default="Videos",
                        help="Subdirectory containing the video files")
    parser.add_argument("--test_fold", type=int, default=9,
                        help="Fold number to use as test set (0-9)")
    parser.add_argument("--val_fold", type=int, default=8,
                        help="Fold number to use as val set (0-9)")
    ...
    if not os.path.exists(filelist_path):
        raise FileNotFoundError(
            ...
            "Make sure --echonet_dir points to the EchoNet-Pediatric root."
        )
    ...
    if ef_col is None:
        ...
        raise ValueError("Cannot find EF/LVEF column")
    ...
    # 4. Split using fold numbers (10-fold CV: integers 0-9)
    #    Use specified folds for val/test, rest for train
    test_df = df[df["Split"] == args.test_fold].copy()
    val_df = df[df["Split"] == args.val_fold].copy()
    train_folds = [i for i in range(10) if i not in (args.test_fold, args.val_fold)]
    ...
    # 6. Write space-delimited CSVs
    for split_name, split_df in [("train", train_df), ("val", val_df), ("test", test_df)]:
        output_path = os.path.join(args.output_dir, f"echonet_pediatric_{split_name}.csv")
    ...
    # 7. Save the scaler
    scaler_path = os.path.join(args.output_dir, "echonet_pediatric_lvef_scaler.pkl")
    with open(scaler_path, "wb") as f:
        pickle.dump(scaler, f)
    print(f"Saved scaler to {scaler_path}")

    # 8. Print config values
    print("\n" + "=" * 50)
